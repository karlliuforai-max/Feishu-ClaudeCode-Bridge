...
> **v0.5.0** · 飞书 ↔ Claude Code / Codex 网关 · [CHANGELOG](./CHANGELOG.md) · [开发文档](./docs/DEVELOPMENT.md)
...
- **引用回复**：用飞书「回复」引用另一条消息再 @ 机器人时，被引用消息的文本与图片/文件会一并拉取并标注为「引用上下文」交给 Agent；被引用消息读取失败（删除/无权限）时降级提示，不影响当前消息。
- **@ 谁**：正文里的 @ 不再整体抹掉，渲染成可读的 `@姓名`，保留「@的是本机器人 / 他人 / 别的机器人」这层信息。
