...
> **当前版本：v0.3.0** · 飞书 Agent Gateway · 见 [CHANGELOG](./CHANGELOG.md)
...
| 图片/文件接收 | ✅ | 附件下载到 `workspace/inbox` 后交给当前 Agent |
...
- `workdir`：agent 的工作目录（cwd），下载附件与生成文件都落在这里；默认是项目根下的 `workspace/`（整目录已 gitignore），可改成别的路径。
...
```bash
python src/feishu_agent_bridge.py
```
...
1. 下载到 `workspace/inbox`。
2. 把本地路径拼进提示词交给当前 Agent。
3. 处理完删除临时文件（启动时也会清扫残留）。
...
```text
src/
  config.py               # 配置加载、运行目录(workspace/inbox/state)、各 Agent 静态配置(AgentConfig)
  agents.py               # Agent 抽象：基类 + ClaudeAgent + CodexAgent + 统一 run() 接口 + 注册表
  feishu_agent_bridge.py  # 飞书 IM 收发、会话持久化与并发隔离、消息派发、进程入口
workspace/                # agent 的 cwd（gitignore）：下载附件与生成文件都在这里
  inbox/                  # 下载的附件，处理后清理
tests/                    # 单元测试
docs/                     # 版本与设计文档
```

依赖方向是单向无环的：`config ← agents ← feishu_agent_bridge`。新增一个 Agent 只需在 `agents.py` 加一个 `Agent` 子类、在 `config.py` 的 `AGENT_CONFIGS` 加一条配置，派发层无需改动。
...
- 运行时版本定义在 `src/feishu_agent_bridge.py` 的 `__version__`。
