...
> **当前版本：v0.0.7** · 核心单文件实现 · 见 [CHANGELOG](./CHANGELOG.md)
...
| 终端流式输出 | ✅ | 终端实时显示 Claude 可见输出，含结构化事件时间线 |
| 流式回复（边生成边更新卡片） | ✅ | 开启 `stream_reply` 后飞书卡片实时打字机式更新 |
| 运行时临时模型切换 | ✅ | `/model` 指令切换会话级模型；`[m:xxx]` 前缀仅本条生效 |
| 群白名单 | ✅ | 可限定只响应指定会话 |
...
```jsonc
{
  ...
  "terminal_stream_format": "text", // text(基础档) | json(结构化事件时间线)
  "stream_reply": false,            // true=飞书卡片边生成边更新(打字机式)；false=生成完一次性回复
  "stream_reply_interval": 0.7,     // 流式卡片最小刷新间隔(秒)，避免触发飞书更新限频
...
- `terminal_stream_format: "json"`：结构化档，使用 `claude -p --output-format stream-json --verbose`，终端以统一时间线展示事件：
  - 工具调用配对：`[tool ▶] Read 文件`（发起）↔ `[tool ✓ 0.3s] Read`（完成，带耗时），失败标 `[tool ✗]`。
  - 常见工具友好摘要：`Read/Write/Edit` 显示文件、`Bash` 显示命令、`Grep/Glob` 显示模式、`WebFetch/WebSearch` 显示 URL/查询。
  - 长工具结果自动折叠（默认前 4 行 / 500 字）；错误、超时、工具失败用 `⚠️` 醒目标记。
  - 每条事件带 `[session 短 ID]` 前缀，多人并发时日志不混杂。
...
开启 `stream_reply: true` 后，桥会用飞书 **CardKit 流式卡片**把 Claude 的输出「边生成边更新」，用户在飞书里能看到打字机式的实时回复，而不必等到全部生成完。

- 默认关闭。关闭时为「生成完一次性回复」的稳妥路径。
- 开启时会强制使用 `stream-json` 以获取增量文本（与 `terminal_stream_format` 无关）。
- `stream_reply_interval`（默认 0.7s）控制卡片最小刷新间隔，避免触发飞书更新限频（50 次/秒、1000 次/分）。
- **稳健兜底**：建卡 / 发卡 / 更新任一环节失败，都会自动退回普通卡片回复，不会丢消息。
- 收尾时卡片写入去掉本地图片引用后的最终文本并关闭流式；本地图片仍作为独立图片消息补发；自定义 `<<<CARD>>>` 卡片不参与流式，自动退回普通透传。

> 前提：飞书应用需开通 CardKit 卡片相关权限（卡片实体的创建与更新）。未开通时会建卡失败并自动退回普通回复。
...
- [x] 丰富 stream-json 结构化事件展示（v0.0.6）：统一时间线、工具调用/结果配对与耗时、常见工具友好摘要、长结果折叠、错误醒目标记、会话前缀
- [x] 流式回复（边生成边更新卡片）（v0.0.6）
- [x] 运行时临时模型切换（v0.0.7）：`/model` 会话级 + `[m:xxx]` 单条前缀，支持短名与完整 model ID
- [ ] 接收语音 / 音视频等其它媒体类型
- [ ] 更细的权限与速率控制
